...
static value return_image(void *data, int ty, int x, int y, int n, int n0)
{
  CAMLparam0();
...
CAMLprim value ml_stbi_load(value channels, value filename)
{
  ...
  n0 = Channels_val(channels);
    ...
    stbi_load(String_val(filename), &x, &y, &n, n0);

  if (image_data)
    ret = return_image(image_data, CAML_BA_UINT8, x, y, n, n0);
  else
    ret = return_failure();
...
CAMLprim value ml_stbi_loadf(value channels, value filename)
{
  ...
  if (image_data)
    ret = return_image(image_data, CAML_BA_FLOAT32, x, y, n, n0);
  else
    ret = return_failure();
...
CAMLprim value ml_stbi_copy(value src, value dst)
{
  CAMLparam2(src, dst);
  const unsigned char *psrc = Caml_ba_data_val(Field(src, 5));
  assert (psrc);
  psrc += Long_val(Field(src, 3));

  unsigned char *pdst = Caml_ba_data_val(Field(dst, 5));
  assert (pdst);
  pdst += Long_val(Field(dst, 3));

  unsigned int
    w = Long_val(Field(src, 0)),
    h = Long_val(Field(src, 1)),
    n = Long_val(Field(src, 2)),
    sstride = Long_val(Field(src, 4)),
    dstride = Long_val(Field(dst, 4)),
    row = w * n;

  assert(w == Long_val(Field(dst, 0)));
  assert(h == Long_val(Field(dst, 1)));
  assert(n == Long_val(Field(dst, 2)));

  for (unsigned int y = 0; y < h; y++)
  {
    memmove(pdst, psrc, row);
    psrc += sstride;
    pdst += dstride;
  }
...
CAMLprim value ml_stbi_copyf(value src, value dst)
{
  CAMLparam2(src, dst);
  const float *psrc = Caml_ba_data_val(Field(src, 5));
  assert (psrc);
  psrc += Long_val(Field(src, 3));

  float *pdst = Caml_ba_data_val(Field(dst, 5));
  assert (pdst);
  pdst += Long_val(Field(dst, 3));
...
#define POUT(i,n) pout[x * n + i] = (pin[2 * x * n + i] + pin[2 * x * n + n + i] + pin[2 * x * n + sin + i] + pin[2 * x * n + sin + n + i]) / 4
#define POUTf(i,n) pout[x * n + i] = (pin[2 * x * n + i] + pin[2 * x * n + n + i] + pin[2 * x * n + sin + i] + pin[2 * x * n + sin + n + i]) / 4.0f

#define LOOP(w,h,n) \
  for (unsigned int y = 0, w2 = (w) / 2, h2 = (h) / 2; \
       y < h2; ++y, pin += 2 * sin, pout += sout) \
...
CAMLprim value ml_stbi_mipmap(value img_in, value img_out)
{
  ...
  unsigned char
    *pin = Caml_ba_data_val(Field(img_in, 5)),
    *pout = Caml_ba_data_val(Field(img_out, 5));
  assert (pin && pout);

  pin += Long_val(Field(img_in, 3));
  pout += Long_val(Field(img_out, 3));
...
CAMLprim value ml_stbi_vflip(value img)
{
  ...
  unsigned char *pbot = ptop + (stride * (h - 1));
